...
In particular:  
1. Follows the original Wadler algebraic formulation, in which concatenation is pairwise, leading to deep document trees.
2. Redundancies in the node types available to the user. This may be a feature to some.
3. Opaque document types.
4. Middling to non-existent documentation for usage. Most libraries assume that you have already read the Wadler papers and then some.

In contrast, this implementation:  
1. Follows an alternate formulation by the Gleam language team, where groups are flattened boxed slices, reducing tree depth.
2. Minimal node types. Most invariants are maintained by witness types and smart constructors.
3. Pattern-matchable documents, with most (but not all) of the contents inspectible and deconstructible.
4. Extensive (if not excessive) documentation.
...
This is not an exhaustive list; you can probably find more.
